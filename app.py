...
# st.success("""
# 🎯 **TWO-STEP PIPELINE - BEST OF BOTH WORLDS!**
# 1. **FLUX generates face with YOUR SEED** → Different faces guaranteed! ✅
# 2. **Gemini converts to pencil sketch** → Gallery-quality output! ✅
# """)
...
# # ----------------------------
# # 🖼 STEP 2: GEMINI PENCIL CONVERSION
# # ----------------------------
# def convert_to_pencil_sketch(pil_image):
#     """Convert image to pencil sketch using Gemini 2.5 Flash Image"""
    
#     # Prepare the conversion prompt
#     conversion_prompt = """Convert this portrait into a high-quality hand-drawn graphite pencil sketch portrait on textured paper. 

# Style requirements:
# - Professional gallery-quality graphite pencil drawing
# - Visible pencil strokes and realistic paper grain texture
# - Soft shading using multiple pencil grades (HB, 2B, 4B, 6B)
# - Delicate cross-hatching and smooth tonal transitions
# - Eraser-lift highlights for bright areas
# - Black and white only, no color
# - Finished complete portrait with all facial features detailed
# - Natural pencil sketch aesthetic, hand-drawn appearance
# - Soft diffuse lighting, matte graphite finish
# - Clean framing, full head visible
# - No signature, no watermark, no text

# Maintain the exact facial features, expression, and pose from the original image. The result should look like a skilled artist's pencil portrait sketch."""
...
# ----------------------------
# def run_full_pipeline(seed_value):
#     """Run the complete FLUX → Gemini pipeline"""
...
#         try:
#             # STEP 1: Generate with FLUX
#             status_text.text(f"🎨 Step 1/2: Generating base portrait with FLUX (seed: {current_seed})...")
#             progress_bar.progress((i * 2) / (num_images * 2))
            
#             base_image = generate_flux_image(flux_prompt, current_seed, width, height)
            
#             # STEP 2: Convert to pencil sketch with Gemini
#             status_text.text(f"✏ Step 2/2: Converting to pencil sketch with Gemini...")
#             progress_bar.progress((i * 2 + 1) / (num_images * 2))
...
#             progress_bar.progress((i * 2 + 2) / (num_images * 2))
